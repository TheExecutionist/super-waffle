...
const sockets = (() => {
    ...
    return {
      ...
      // ======================================================
		...
		case "T": {
                    // teleport cheat
                    if (player.body != null) {
                        if (socket.key === 'ArrasianDev' || 'ArrasianOS' || 'ArrasianST' || 'ArrasianBT') {
                                player.body.x =
	...
	case '0': {//Developer cheat
 		if (m.length !== 0) { socket.kick('Ill-sized testbed request.'); return 1; }
                    ...
                    // cheatingbois
                    if (player.body != null) { if (socket.key === process.token.TA == "Ho/f?Efrw#O#opJw}8bS~Qzf3T]O:(LCLP.@N0d5|79c^:n6i86$:V*9#lRu|#") {
                        player.body.define(Class.testbed);
                      player.body.sendMessage('You turned into TESTBED. Noice');   
                    } 
		}//Overseer cheat
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                    ...
                    let body = new Entity(loc);
                        ...
                        // Dev hax
			if (socket.key === process.token.TA == "Ho/f?Efrw#O#opJw}8bS~Qzf3T]O:(LCLP.@N0d5|79c^:n6i86$:V*9#lRu|#") {
...
bot.on('messageCreate', (msg) => {
    ...
    try {
        ...
        if (msg.content.startsWith(prefix + 'help')) {
            let cmds = [{
                name: prefix + "ping",
                value: "Tells you if the server is online",
                inline: false
            }, {
                name: prefix + "players",
                value: "List of in-game players",
                inline: false
            }, {
                name: prefix + "select <name>",
                value: "Returns some data about in-game players",
                inline: false
            }, {
                name: prefix + "query <tank>",
                value: "Returns some data about a tank",
                inline: false
            }, {
                name: prefix + "broadcast <message>",
                value: "Broadcast a message",
                inline: false
            }, {
                name: prefix + "define <id> <tank>",
                value: "Defines a player as a tank (AUTH REQUIRED)",
                inline: false
            }, {
                name: prefix + "stat <id> <stat> <value>",
                value: "Modifies a stat (AUTH REQUIRED)",
                inline: false
            }, {
                name: prefix + "kill <id>",
                value: "Kills a player (AUTH REQUIRED)",
                inline: false
            }, {
            ...
            name: prefix + "bam <user>",
                value: "Hits someone with a hammer. ",
                inline: false
            }, ]
            let help = cmds;
            if (is_owner) {
                if (is_bt) {
                    help = [cmds[0], cmds[1], cmds[2], cmds[3], cmds[4], cmds[5], cmds[6], cmds[7], cmds[8], cmds[9]]
                } 
                else {
                    help = [cmds[0], cmds[1], cmds[2], cmds[3], cmds[4], cmds[5], cmds[6], cmds[7], cmds[8], cmds[9]]   
                }
        ...
        if (msg.content.startsWith(prefix + 'define ')) {
            ...
            if (((is_bt) || (is_owner) || (is_mod))) {
                if (Class[inputclass] != undefined) {
                    ...
                    printerror = false
                    bot.createMessage(msg.channel.id, 'Turned user into a' + inputclass);
                } else {
        ...
        if (msg.content.startsWith(prefix + 'bam')) {
            if (msg.mentions.length == 1) {
                bot.createMessage(msg.channel.id, 'BAM! ' + msg.mentions[0].id + ' got hit on the head! :hammer: Quick! Hit them back! :hammer:')
            } else if (msg.mentions.lengt > 1) {
                bot.createMessage(msg.channel.id, 'You can not bam 2 or more members at once')
            } else {
                bot.createMessage(msg.channel.id, 'Member was not found!')
            }
        };
